...
export default function Arbre() {
  ...
  const styles = {
    ...
    btnSideAdd: { width: '100%', background: '#9B6240', color: '#FFF', border: 'none', padding: '8px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', marginTop: '12px' },
    ...
    noeud: { background: darkMode ? '#16213e' : '#FFF9F3', border: `1px solid ${darkMode ? '#e94560' : '#E8C9A0'}`, borderRadius: '12px', padding: '12px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px', minWidth: '160px', marginBottom: '4px' },
    ...
    noeudInfo: { flex: 1 },
    noeudNom: { fontSize: '14px', fontWeight: '500', color: darkMode ? '#e0e0e0' : '#3D2410' },
    noeudAnnees: { fontSize: '11px', color: darkMode ? '#a0a0a0' : '#B08060', marginTop: '2px' },
    fichePopup: { background: darkMode ? '#16213e' : '#FFF', border: `1px solid ${darkMode ? '#e94560' : '#E8C9A0'}`, borderRadius: '12px', padding: '12px', marginBottom: '8px', minWidth: '200px', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' },
    ficheHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' },
    ficheLigne: { fontSize: '13px', color: darkMode ? '#a0a0a0' : '#7A5035', marginBottom: '4px' },
    btnSupprimerFiche: { background: 'none', border: 'none', color: '#C06060', cursor: 'pointer', fontSize: '12px' },
    btnAjouterEnfant: { background: '#9B6240', color: '#FFF', border: 'none', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', marginTop: '8px' },
    enfantsWrap: { display: 'flex', flexDirection: 'column', alignItems: 'center' },
    ligneVerticale: { width: '2px', height: '30px', background: darkMode ? '#e94560' : '#E8C9A0' },
  ...
  const couleurs = [{ bg: '#E8C9A0', color: '#6B3F20' }, { bg: '#C8D8E8', color: '#203060' }, { bg: '#C8E0C8', color: '#2A6030' }, { bg: '#E8C8D8', color: '#601840' }, { bg: '#D8C8E0', color: '#402060' }]
    ...
    <div style={styles.page}>
      <nav style={styles.nav}>
        ...
        <div style={styles.navLinks}>
          ...
          <button style={{ ...styles.navBtn, ...styles.navBtnActive }}>Arbre</button>
      ...
      <div style={styles.app}>
        <div style={styles.sidebar}>
          ...
          <div style={{ ...styles.sideItem, ...styles.sideItemActive }}>🌳 Arbre</div>
          ...
          <button onClick={() => setShowForm(!showForm)} style={styles.btnSideAdd}>{showForm ? 'Annuler' : '+ Ajouter'}</button>
        </div>

        <div style={styles.main}>
          <div style={styles.header}><h1 style={styles.titre}>🌳 Arbre généalogique</h1><p style={styles.sousTitre}>{membres.length} membre{membres.length > 1 ? 's' : ''} · {utilisateur.famille}</p></div>
